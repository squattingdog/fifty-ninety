...
import { EVENT_PROJECT_CREATED } from 'c/constants';

export default class ProjectSheetCreate extends LightningElement {
    @track projectSheetRecordTypeId;
    @track showModal;
    @track errorMessage;
        ...
        try {
            if(result) {
                if(result.error) {
                    ...
                    this.projectSheetRecordTypeId = Object.keys(rtis).find(rti => rtis[rti].name === 'Sheet');
                } else {
                    // handle this condition
                }
            } else {
    ...
    onCancelNewSheetClicked(event) {
        console.log('onCancelNewProjectClicked');
    ...
    onCreateSheetSuccess(event) {
        ...
        // fireEvent(this.pageRef, EVENT_PROJECT_CREATED, event.detail);
    }
}